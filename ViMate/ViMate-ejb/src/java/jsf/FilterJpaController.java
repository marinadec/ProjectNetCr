...
public class FilterJpaController implements Serializable {
    ...
    public FilterJpaController(UserTransaction utx, EntityManagerFactory emf) {
        this.utx = utx;
        this.emf = emf;
    }
    ...
    public void create(Filter filter) throws RollbackFailureException, Exception {
    ...
    public void edit(Filter filter) throws NonexistentEntityException, RollbackFailureException, Exception {
        EntityManager em = null;
        try {
            utx.begin();
            em = getEntityManager();
            ...
            if (msg == null || msg.length() == 0) {
                ...
                if (findFilter(id) == null) {
                    throw new NonexistentEntityException("The filter with id " + id + " no longer exists.");
                }
            }
            throw ex;
        } finally {
    ...
    public void destroy(Long id) throws NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            try {
                ...
                throw new NonexistentEntityException("The filter with id " + id + " no longer exists.", enfe);
            }
            em.remove(filter);
            utx.commit();
        } catch (Exception ex) {
    ...
    private List<Filter> findFilterEntities(boolean all, int maxResults, int firstResult) {
        ...
        try {
            ...
            cq.select(cq.from(Filter.class));
    ...
    public int getFilterCount() {
        ...
        try {
            CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
            Root<Filter> rt = cq.from(Filter.class);
